  ...
  return (
    <EcdShell musicBed={0.04} showSound={false}>
      ...
      <div className="relative z-10 flex w-full flex-1 flex-col items-center px-4 pb-16 pt-4 sm:pt-6 lg:pt-4">
        ...
        {/* the whole alphabet, so any letter can be practised on its own — glassmorphic,
            one continuous line that scrolls, sitting naturally in the layout rather
        {/* rail — glassmorphic, one row on mobile (scrolls), stretches to fit every
            letter with no scroll on larger screens — same pattern as Meet the Letters */}
        <div className="mt-3 flex w-full max-w-[820px] flex-nowrap items-center gap-3 overflow-x-auto scroll-smooth rounded-2xl border border-white/30 bg-white/15 px-3 py-2.5 shadow-[0_8px_32px_rgba(0,40,60,0.18)] backdrop-blur-md sm:gap-3.5 sm:px-4 lg:max-w-[1200px] lg:overflow-visible lg:gap-3 lg:px-4 lg:py-2.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {PHONICS_ALPHABET.map((item, position) => {
            ...
            return (
              <button
                ...
                className={`relative flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-[16px] transition-all duration-200 hover:scale-110 sm:h-12 sm:w-12 sm:text-[18px] lg:aspect-square lg:h-auto lg:w-auto lg:min-w-0 lg:max-w-[54px] lg:flex-1 lg:shrink lg:basis-0 lg:text-[20px] ${
                  isCurrent
