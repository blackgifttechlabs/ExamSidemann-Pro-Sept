...
import { ArrowLeft, Check, X, Trophy, Flame, ArrowRight, Lightbulb, RotateCcw } from 'lucide-react';
import { MATH_QUESTIONS, type MathQuestion } from '../data/mathPuzzlesData';
...
export const MathTrainer: React.FC<MathTrainerProps> = ({ onBack, onAddXp }) => {
  const [questions] = useState<MathQuestion[]>(MATH_QUESTIONS);
    ...
    <div className="w-full max-w-3xl mx-auto flex flex-col items-center">
      ...
      <div className="w-full flex items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="flex items-center gap-1 px-3 py-1 bg-purple-50 dark:bg-purple-950/40 border border-purple-200 dark:border-purple-800/60 rounded-lg text-purple-700 dark:text-purple-300 text-xs font-black">
            <Trophy size={14} />
      ...
      <div className="w-full mb-6">
        <div className="flex justify-between items-center text-xs font-bold text-slate-500 dark:text-slate-400 mb-1.5">
          ...
          <span className="text-purple-600 dark:text-purple-400 uppercase tracking-wider">{currentQ.category} • {currentQ.difficulty}</span>
        </div>
      ...
      <div className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl">
        <div className="flex items-center justify-between mb-4">
          <span className="px-3 py-1 bg-purple-100 dark:bg-purple-950/60 text-purple-700 dark:text-purple-300 rounded-full text-xs font-black uppercase tracking-wider">
            {currentQ.category}
          </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
          {currentQ.options.map((option, idx) => {
            ...
            let buttonStyle = 'bg-slate-50 dark:bg-slate-800/70 border-slate-200 dark:border-slate-700 hover:border-purple-400 text-slate-800 dark:text-white';
            if (isAnswered) {
              if (isThisCorrect) {
        ...
        {isAnswered && (
          <div className="mt-4 p-4 rounded-xl bg-purple-50 dark:bg-purple-950/30 border border-purple-200 dark:border-purple-800 animate-fadeIn">
        ...
        {isAnswered && (
          <div className="mt-6 flex justify-end">
            <button
              onClick={handleNext}
              className="px-6 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-black text-xs uppercase tracking-wider flex items-center gap-2 shadow-lg transition-all active:scale-95"
            >
