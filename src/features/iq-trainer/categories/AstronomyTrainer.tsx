...
import { ArrowLeft, Check, X, Trophy, Flame, ArrowRight, Sparkles } from 'lucide-react';
import { ASTRONOMY_QUESTIONS, type AstronomyQuestion } from '../data/astronomyQuestionsData';

interface AstronomyTrainerProps {
  onBack: () => void;
  onAddXp?: (xp: number) => void;
  soundEnabled?: boolean;
}

export const AstronomyTrainer: React.FC<AstronomyTrainerProps> = ({ onBack, onAddXp }) => {
  const [questions] = useState<AstronomyQuestion[]>(ASTRONOMY_QUESTIONS);
    ...
    <div className="w-full max-w-3xl mx-auto flex flex-col items-center">
      ...
      <div className="w-full flex items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="flex items-center gap-1 px-3 py-1 bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-800/60 rounded-lg text-indigo-700 dark:text-indigo-300 text-xs font-black">
            <Trophy size={14} />
      ...
      <div className="w-full mb-6">
        <div className="flex justify-between items-center text-xs font-bold text-slate-500 dark:text-slate-400 mb-1.5">
          ...
          <span className="text-indigo-600 dark:text-indigo-400 uppercase tracking-wider">{currentQ.topic}</span>
        </div>
      ...
      <div className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
          {currentQ.options.map((option, idx) => {
            ...
            let buttonStyle = 'bg-slate-50 dark:bg-slate-800/70 border-slate-200 dark:border-slate-700 hover:border-indigo-400 text-slate-800 dark:text-white';
            if (isAnswered) {
              if (isThisCorrect) {
        ...
        {isAnswered && (
          <div className="mt-4 p-4 rounded-xl bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-800 animate-fadeIn">
            ...
            <div className="flex items-start gap-2 text-xs text-slate-700 dark:text-slate-300 leading-relaxed">
              ...
              <p><strong>Cosmic Fact:</strong> {currentQ.explanation}</p>
            </div>
          </div>
        )}
        ...
        {isAnswered && (
          <div className="mt-6 flex justify-end">
            <button
              onClick={handleNext}
              className="px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-black text-xs uppercase tracking-wider flex items-center gap-2 shadow-lg transition-all active:scale-95"
            >
