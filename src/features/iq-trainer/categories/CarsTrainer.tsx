...
import { ArrowLeft, Check, X, Trophy, Flame, ArrowRight, Gauge } from 'lucide-react';
import { CARS_QUESTIONS, type CarQuestion } from '../data/carsQuestionsData';

interface CarsTrainerProps {
  onBack: () => void;
  onAddXp?: (xp: number) => void;
  soundEnabled?: boolean;
}

export const CarsTrainer: React.FC<CarsTrainerProps> = ({ onBack, onAddXp }) => {
  const [questions] = useState<CarQuestion[]>(CARS_QUESTIONS);
    ...
    <div className="w-full max-w-3xl mx-auto flex flex-col items-center">
      ...
      <div className="w-full mb-6">
        <div className="flex justify-between items-center text-xs font-bold text-slate-500 dark:text-slate-400 mb-1.5">
          ...
          <span className="text-rose-600 dark:text-rose-400 uppercase tracking-wider">{currentQ.category}</span>
        </div>
        <div className="w-full h-2 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-rose-500 to-amber-600 transition-all duration-300 rounded-full"
      ...
      <div className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl">
        <div className="flex items-center justify-between mb-4">
          <span className="px-3 py-1 bg-rose-100 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 rounded-full text-xs font-black uppercase tracking-wider">
            {currentQ.category}
          </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
          {currentQ.options.map((option, idx) => {
            ...
            let buttonStyle = 'bg-slate-50 dark:bg-slate-800/70 border-slate-200 dark:border-slate-700 hover:border-rose-400 text-slate-800 dark:text-white';
            if (isAnswered) {
              if (isThisCorrect) {
        ...
        {isAnswered && (
          <div className="mt-4 p-4 rounded-xl bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-800 animate-fadeIn">
            ...
            <div className="flex items-start gap-2 text-xs text-slate-700 dark:text-slate-300 leading-relaxed">
              <Gauge size={16} className="text-rose-500 shrink-0 mt-0.5" />
              <p><strong>Auto Spec Fact:</strong> {currentQ.explanation}</p>
            </div>
          </div>
        )}
        ...
        {isAnswered && (
          <div className="mt-6 flex justify-end">
            <button
              onClick={handleNext}
              className="px-6 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-black text-xs uppercase tracking-wider flex items-center gap-2 shadow-lg transition-all active:scale-95"
            >
