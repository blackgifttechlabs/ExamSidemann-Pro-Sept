import React, { useState, useEffect, useRef, memo, useMemo } from 'react';
import { useLessonState } from '../../../lessonProgress';
import {
  BookOpen,
  Search,
  X,
  ChevronUp,
  RefreshCw,
  Sparkles,
  GraduationCap,
  Trophy,
  Layers,
  Target,
  AlignLeft,
  FileText,
  Edit3,
  CheckCircle,
  HelpCircle,
  ArrowRight,
  ShieldCheck,
  Info,
  ClipboardList,
  Type,
  Repeat,
  MessageSquare,
  Zap,
} from 'lucide-react';

// ──────────────────────────────────────────────────────────────────────────────
// INTERFACES
// ──────────────────────────────────────────────────────────────────────────────
interface Question {
  id: number;
  question: string;
  answer: React.ReactNode;
}

interface Section {
  id: string;
  title: string;
  subtitle?: string;
  description: string;
  details: string[];
  examples: React.ReactNode;
  questions: Question[];
}

// ──────────────────────────────────────────────────────────────────────────────
// DATA: Vocabulary Development Sections
// ──────────────────────────────────────────────────────────────────────────────
const SECTIONS_DATA: Section[] = [
  {
    id: 'wordbuilding',
    title: 'Word Building (The Construction Site)',
    description:
      'English words are often made of different parts. If you learn the parts, you can understand words you have never seen before. It is like using Lego blocks – start with a root word and add prefixes or suffixes to change the meaning.',
    details: [
      'Root Words: The most basic part of a word; the main meaning (e.g., "play" in "player").',
      'Prefixes: Added to the beginning to change meaning (un-, re-, pre-, mis-, dis-).',
      'Suffixes: Added to the end to change meaning or part of speech (-tion, -ness, -ful, -less, -ly).',
      'Compound Words: Two whole words joined together (sun + flower = sunflower).',
    ],
    examples: (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm">
          <h5 className="text-sm font-bold text-indigo-600 dark:text-indigo-400 uppercase">Root Words</h5>
          <p className="text-xs text-slate-600 dark:text-slate-400">In "player," the root is "play." In "unhappy," the root is "happy."</p>
        </div>
        <div className="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm">
          <h5 className="text-sm font-bold text-purple-600 dark:text-purple-400 uppercase">Prefixes</h5>
          <ul className="text-xs text-slate-600 dark:text-slate-400 space-y-1">
            <li><strong>un-</strong> (NOT): unkind</li>
            <li><strong>re-</strong> (AGAIN): refill</li>
            <li><strong>pre-</strong> (BEFORE): preview</li>
            <li><strong>mis-</strong> (WRONGLY): mislead</li>
            <li><strong>dis-</strong> (OPPOSITE): disobey</li>
          </ul>
        </div>
        <div className="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm">
          <h5 className="text-sm font-bold text-orange-600 dark:text-orange-400 uppercase">Suffixes</h5>
          <ul className="text-xs text-slate-600 dark:text-slate-400 space-y-1">
            <li><strong>-tion</strong> (Noun): education</li>
            <li><strong>-ness</strong> (State): sadness</li>
            <li><strong>-ful</strong> (Full of): careful</li>
            <li><strong>-less</strong> (Without): hopeless</li>
            <li><strong>-ly</strong> (Manner): slowly</li>
          </ul>
        </div>
        <div className="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm">
          <h5 className="text-sm font-bold text-green-600 dark:text-green-400 uppercase">Compound Words</h5>
          <p className="text-xs text-slate-600 dark:text-slate-400">Sun + flower = Sunflower<br />Rain + coat = Raincoat<br />Note + book = Notebook</p>
        </div>
      </div>
    ),
    questions: [
      {
        id: 1,
        question: 'What is the root word in "disagreement"?',
        answer: 'Agree.',
      },
      {
        id: 2,
        question: 'Add a prefix to the word "trust" to show you do NOT trust someone.',
        answer: 'Distrust.',
      },
      {
        id: 3,
        question: 'Add a suffix to "help" to describe someone who helps others a lot.',
        answer: 'Helpful.',
      },
      {
        id: 4,
        question: 'Join "back" and "pack" to make a compound word.',
        answer: 'Backpack.',
      },
      {
        id: 5,
        question: 'What does the word "rewrite" mean?',
        answer: 'To write again.',
      },
    ],
  },
  {
    id: 'relationships',
    title: 'Synonyms, Antonyms, Homophones, and Homonyms',
    description:
      'Understanding word relationships helps you choose the right word for the right context. These four categories are essential for vocabulary development.',
    details: [
      'Synonyms: Words that mean the same (big/large, small/tiny).',
      'Antonyms: Words that mean the opposite (hot/cold, tall/short).',
      'Homophones: Sound the same but have different spelling and meaning (sun/son, there/their).',
      'Homonyms: Spelled the same but have different meanings (bark – dog sound vs. tree skin).',
    ],
    examples: (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm">
          <h5 className="text-sm font-bold text-indigo-600 dark:text-indigo-400 uppercase">Synonyms</h5>
          <p className="text-xs text-slate-600 dark:text-slate-400">Big / Large; Small / Tiny; Fast / Quick; Sick / Ill.</p>
        </div>
        <div className="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm">
          <h5 className="text-sm font-bold text-red-600 dark:text-red-400 uppercase">Antonyms</h5>
          <p className="text-xs text-slate-600 dark:text-slate-400">Hot / Cold; Tall / Short; Rich / Poor; Happy / Sad.</p>
        </div>
        <div className="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm">
          <h5 className="text-sm font-bold text-orange-600 dark:text-orange-400 uppercase">Homophones</h5>
          <p className="text-xs text-slate-600 dark:text-slate-400">Sun (star) / Son (child); There (place) / Their (belongs).</p>
        </div>
        <div className="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm">
          <h5 className="text-sm font-bold text-green-600 dark:text-green-400 uppercase">Homonyms</h5>
          <p className="text-xs text-slate-600 dark:text-slate-400">Bark (dog sound) / Bark (tree skin); Bank (money) / Bank (river side).</p>
        </div>
      </div>
    ),
    questions: [
      {
        id: 1,
        question: 'What is a synonym for "difficult"?',
        answer: 'Hard.',
      },
      {
        id: 2,
        question: 'What is the antonym of "heavy"?',
        answer: 'Light.',
      },
      {
        id: 3,
        question: 'Choose the correct homophone: I can (hear / here) the birds singing.',
        answer: 'Hear.',
      },
      {
        id: 4,
        question: 'The word "bat" can be a wooden stick for sports or a flying animal. Is this a homonym or a homophone?',
        answer: 'Homonym.',
      },
      {
        id: 5,
        question: 'Write the antonym of "expensive".',
        answer: 'Cheap.',
      },
    ],
  },
  {
    id: 'contextual',
    title: 'Contextual Vocabulary (Words for Different Places)',
    description:
      'Different situations require different vocabulary. Academic words help you in school, while topic‑specific words help you discuss subjects like the environment, technology, health, and education.',
    details: [
      'Academic Vocabulary: analyze, define, compare, contrast – used in exams and essays.',
      'Environment: pollution, nature, conservation, climate, wildlife.',
      'Technology: computer, internet, software, device, digital.',
      'Health: disease, clinic, nutrition, hygiene, medicine.',
      'Education: syllabus, student, primary, secondary, university.',
    ],
    examples: (
      <div className="space-y-4">
        <div className="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm">
          <h5 className="text-sm font-bold text-indigo-600 dark:text-indigo-400 uppercase">Academic Vocabulary</h5>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs text-slate-600 dark:text-slate-400">
            <span><strong>Analyze:</strong> Look at carefully to understand.</span>
            <span><strong>Define:</strong> Give the exact meaning.</span>
            <span><strong>Compare:</strong> Show similarities.</span>
            <span><strong>Contrast:</strong> Show differences.</span>
          </div>
        </div>
        <div className="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm">
          <h5 className="text-sm font-bold text-purple-600 dark:text-purple-400 uppercase">Topic‑Based Vocabulary</h5>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs text-slate-600 dark:text-slate-400">
            <div><span className="font-bold">Environment:</span> pollution, nature</div>
            <div><span className="font-bold">Technology:</span> computer, internet</div>
            <div><span className="font-bold">Health:</span> disease, clinic</div>
            <div><span className="font-bold">Education:</span> syllabus, student</div>
          </div>
        </div>
      </div>
    ),
    questions: [
      {
        id: 1,
        question: 'In which subject would you use the word "pollution"?',
        answer: 'Geography / Science (or Environment).',
      },
      {
        id: 2,
        question: 'What does it mean to "compare" two things?',
        answer: 'To show how they are the same.',
      },
      {
        id: 3,
        question: 'If you are at a "clinic," are you talking about Health or Technology?',
        answer: 'Health.',
      },
      {
        id: 4,
        question: 'Give one word related to Education.',
        answer: '(Example: Teacher, School, Book, Exam).',
      },
      {
        id: 5,
        question: 'If a teacher asks you to "define" a word, what should you do?',
        answer: 'Give the meaning of the word.',
      },
    ],
  },
  {
    id: 'formal',
    title: 'Formal vs. Informal Language',
    description:
      'Knowing when to use formal or informal language is essential for effective communication. Formal language is used in academic and professional settings; informal language is for friends and family.',
    details: [
      'Formal: Polite, uses full words, avoids contractions. (e.g., "I would like to request some water.")',
      'Informal: Casual, uses slang and contractions. (e.g., "Hey, give me some water.")',
      'Use formal for exams, letters to authorities, and professional communication.',
      'Use informal for personal messages, conversations with friends, and social media.',
    ],
    examples: (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 bg-white dark:bg-slate-800 rounded-xl shadow-sm">
          <h5 className="text-sm font-bold text-blue-600 dark:text-blue-400 uppercase">Formal</h5>
          <p className="text-xs text-slate-600 dark:text-slate-400">"I would like to request some water, please."</p>
        </div>
        <div className="p-4 bg-white dark:bg-slate-800 rounded-xl shadow-sm">
          <h5 className="text-sm font-bold text-slate-600 dark:text-slate-400 uppercase">Informal</h5>
          <p className="text-xs text-slate-600 dark:text-slate-400">"Hey, give me some water."</p>
        </div>
      </div>
    ),
    questions: [
      {
        id: 1,
        question: 'Is "What\'s up, bro?" formal or informal?',
        answer: 'Informal.',
      },
      {
        id: 2,
        question: 'Which is better for an exam: "I\'m gonna" or "I am going to"?',
        answer: '"I am going to".',
      },
      {
        id: 3,
        question: 'Which formal: "How do you do?" or "How\'s it going?"',
        answer: '"How do you do?"',
      },
      {
        id: 4,
        question: 'Would you use informal language when writing a letter to the Headmaster?',
        answer: 'No (Always use formal language with authority).',
      },
      {
        id: 5,
        question: 'Rewrite this informal sentence formally: "Give me your pen."',
        answer: '"May I please borrow your pen?"',
      },
    ],
  },
  {
    id: 'idioms',
    title: 'Idioms and Common Expressions',
    description:
      'An idiom is a group of words that has a special meaning different from the literal meaning. You cannot translate them word‑for‑word. They add colour to your language.',
    details: [
      'A piece of cake: Something very easy.',
      'Under the weather: Feeling sick.',
      'Once in a blue moon: Very rarely.',
      'Break a leg: Good luck!',
      'Hit the nail on the head: To be exactly right.',
    ],
    examples: (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="p-4 bg-white dark:bg-slate-800 border border-indigo-100 dark:border-indigo-800 rounded-xl shadow-sm">
          <h5 className="text-[10px] font-bold text-blue-600 dark:text-blue-400 uppercase">A piece of cake</h5>
          <p className="text-xs text-slate-600 dark:text-slate-400">Something very easy.</p>
          <p className="text-[10px] italic text-slate-500">"That exam was a piece of cake!"</p>
        </div>
        <div className="p-4 bg-white dark:bg-slate-800 border border-indigo-100 dark:border-indigo-800 rounded-xl shadow-sm">
          <h5 className="text-[10px] font-bold text-purple-600 dark:text-purple-400 uppercase">Under the weather</h5>
          <p className="text-xs text-slate-600 dark:text-slate-400">Feeling sick.</p>
          <p className="text-[10px] italic text-slate-500">"I am under the weather today."</p>
        </div>
        <div className="p-4 bg-white dark:bg-slate-800 border border-indigo-100 dark:border-indigo-800 rounded-xl shadow-sm">
          <h5 className="text-[10px] font-bold text-orange-600 dark:text-orange-400 uppercase">Once in a blue moon</h5>
          <p className="text-xs text-slate-600 dark:text-slate-400">Very rarely.</p>
          <p className="text-[10px] italic text-slate-500">"He cleans his room once in a blue moon."</p>
        </div>
      </div>
    ),
    questions: [
      {
        id: 1,
        question: 'If a task is "a piece of cake," is it hard or easy?',
        answer: 'Easy.',
      },
      {
        id: 2,
        question: 'What does it mean if I say, "I am under the weather"?',
        answer: 'You are sick/ill.',
      },
      {
        id: 3,
        question: 'If I visit the city "once in a blue moon," do I go there often?',
        answer: 'No, very rarely.',
      },
      {
        id: 4,
        question: 'Choose the correct idiom: Winning the race was (a piece of cake / under the weather).',
        answer: 'A piece of cake.',
      },
      {
        id: 5,
        question: 'What does the idiom "break a leg" mean?',
        answer: 'It means "Good luck!"',
      },
    ],
  },
  {
    id: 'proverbs',
    title: 'Proverbs and Their Meanings',
    description:
      'A proverb is a short, well‑known saying that gives advice or tells a truth about life. In Zimbabwe, we value proverbs because they show wisdom.',
    details: [
      'Unity is strength: People are more powerful when they work together.',
      'A stitch in time saves nine: Fix a small problem now to avoid a bigger one later.',
      'Actions speak louder than words: What you do is more important than what you say.',
      'Early bird catches the worm: Starting early leads to success.',
      'Practice makes perfect: Doing something repeatedly makes you better.',
    ],
    examples: (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 bg-white dark:bg-slate-800 border border-indigo-100 dark:border-indigo-800 rounded-xl shadow-sm">
          <h5 className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 uppercase">Unity is strength</h5>
          <p className="text-xs text-slate-600 dark:text-slate-400">People are more powerful when they work together.</p>
        </div>
        <div className="p-4 bg-white dark:bg-slate-800 border border-indigo-100 dark:border-indigo-800 rounded-xl shadow-sm">
          <h5 className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 uppercase">A stitch in time saves nine</h5>
          <p className="text-xs text-slate-600 dark:text-slate-400">Fix a small problem now to avoid a big problem later.</p>
        </div>
        <div className="p-4 bg-white dark:bg-slate-800 border border-indigo-100 dark:border-indigo-800 rounded-xl shadow-sm">
          <h5 className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 uppercase">Actions speak louder than words</h5>
          <p className="text-xs text-slate-600 dark:text-slate-400">What you do is more important than what you say.</p>
        </div>
        <div className="p-4 bg-white dark:bg-slate-800 border border-indigo-100 dark:border-indigo-800 rounded-xl shadow-sm">
          <h5 className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 uppercase">Early bird catches the worm</h5>
          <p className="text-xs text-slate-600 dark:text-slate-400">Starting early leads to success.</p>
        </div>
      </div>
    ),
    questions: [
      {
        id: 1,
        question: 'Which proverb tells us that working together is good?',
        answer: 'Unity is strength.',
      },
      {
        id: 2,
        question: 'What does "Early bird catches the worm" mean?',
        answer: 'It means starting early leads to success.',
      },
      {
        id: 3,
        question: 'If someone says they will help you, but they never do, which proverb fits?',
        answer: 'Actions speak louder than words.',
      },
      {
        id: 4,
        question: 'If your clothes have a small hole and you fix it today, which proverb are you following?',
        answer: 'A stitch in time saves nine.',
      },
      {
        id: 5,
        question: 'Complete the proverb: "Unity is ________."',
        answer: 'Strength.',
      },
    ],
  },
  {
    id: 'final',
    title: 'Final Assessment',
    subtitle: 'Put it all together',
    description:
      'Read the short story below and answer the questions that follow. Apply all the vocabulary skills you have learned: word building, synonyms/antonyms, homophones, formal/informal language, idioms, and proverbs.',
    details: [
      'Read the passage carefully.',
      'Identify compound words, root words with suffixes, idioms, and proverbs.',
      'Recognise proper nouns from previous lessons.',
      'Explain meanings of phrases in context.',
    ],
    examples: (
      <div className="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl italic font-serif">
        <h5 className="not-italic font-bold text-xs text-indigo-600 dark:text-indigo-400 uppercase tracking-widest mb-2">
          The Helpful Boy
        </h5>
        <p className="text-sm leading-relaxed text-slate-700 dark:text-slate-300">
          "Tapuwa was a helpful boy. He lived in a house with a red roof near the Zambezi River. One day, he felt under the weather and couldn't go to school. His mother told him, 'Rest now, because a stitch in time saves nine.' He lay in bed and read his notebook until he felt better. He knew that passing his exams would be a piece of cake if he studied hard."
        </p>
      </div>
    ),
    questions: [
      {
        id: 1,
        question: 'Identify a compound word in the story.',
        answer: 'Notebook (or "Zambezi River" – though that is a phrase, "notebook" is the compound word).',
      },
      {
        id: 2,
        question: 'Find a word with a suffix. What is the root word?',
        answer: 'Helpful. Root word is Help.',
      },
      {
        id: 3,
        question: 'Identify the idiom used to describe Tapuwa\'s health.',
        answer: 'Under the weather (meaning he was sick).',
      },
      {
        id: 4,
        question: 'There is a Proper Noun in this story. What is it?',
        answer: 'Tapuwa or Zambezi River.',
      },
      {
        id: 5,
        question: 'Which proverb did the mother use? What does it mean?',
        answer: '"A stitch in time saves nine." It means he should rest now to prevent getting even sicker.',
      },
    ],
  },
];

// ──────────────────────────────────────────────────────────────────────────────
// SECTION TABS FOR NAVIGATION
// ──────────────────────────────────────────────────────────────────────────────
const SECTION_TABS = SECTIONS_DATA.map((s) => ({ id: s.id, label: s.title.split(' ')[0] }));

// ──────────────────────────────────────────────────────────────────────────────
// INTERACTIVE QUESTION COMPONENT
// ──────────────────────────────────────────────────────────────────────────────
const InteractiveQuestion = memo(
  ({
    question,
    answer,
    index,
  }: {
    question: string;
    answer: React.ReactNode;
    index: number;
  }) => {
    const [isRevealed, setIsRevealed] = useState(false);

    return (
      <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#121212] p-4 shadow-sm transition-all hover:shadow-md">
        <div className="flex items-center justify-between mb-2">
          <span className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">
            Question {index}
          </span>
          <button
            onClick={() => setIsRevealed(!isRevealed)}
            className={`flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider transition-colors ${
              isRevealed
                ? 'text-green-600 dark:text-green-400'
                : 'text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300'
            }`}
          >
            {isRevealed ? (
              <>
                <CheckCircle size={14} /> Answer
              </>
            ) : (
              <>
                <HelpCircle size={14} /> Reveal
              </>
            )}
          </button>
        </div>
        <p className="text-sm font-medium text-slate-800 dark:text-slate-200 leading-relaxed">
          {question}
        </p>
        {isRevealed && (
          <div className="mt-3 animate-dropdown-reveal rounded-lg bg-green-50 dark:bg-green-900/20 p-3">
            <div className="text-sm font-semibold text-green-700 dark:text-green-400">
              {answer}
            </div>
          </div>
        )}
      </div>
    );
  },
  (prev, next) => prev.index === next.index && prev.question === next.question
);

// ──────────────────────────────────────────────────────────────────────────────
// SECTION CARD COMPONENT (memoized)
// ──────────────────────────────────────────────────────────────────────────────
const SectionCard = memo(
  ({ section, isHighlighted }: { section: Section; isHighlighted: boolean }) => {
    return (
      <div
        id={`section-${section.id}`}
        className={`rounded-xl border p-4 md:p-6 shadow-sm transition-all duration-300 ease-out hover:shadow-md ${
          isHighlighted
            ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 ring-2 ring-indigo-500/50 scale-[1.01]'
            : 'border-slate-200 dark:border-slate-800 bg-white dark:bg-[#121212] hover:border-indigo-300 dark:hover:border-indigo-700'
        }`}
      >
        <h3
          className={`text-xl md:text-2xl font-bold mb-1 ${
            isHighlighted
              ? 'text-indigo-900 dark:text-indigo-100'
              : 'text-slate-900 dark:text-slate-100'
          }`}
        >
          {section.title}
        </h3>
        {section.subtitle && (
          <p className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest mb-3">
            {section.subtitle}
          </p>
        )}
        <p className="text-sm md:text-base text-slate-700 dark:text-slate-300 leading-relaxed mb-4">
          {section.description}
        </p>

        {/* Details as bullet list */}
        <div className="mb-4 rounded-lg bg-slate-50 dark:bg-white/5 p-3 border border-slate-100 dark:border-white/5">
          <ul className="space-y-1 text-sm text-slate-600 dark:text-slate-400">
            {section.details.map((detail, idx) => (
              <li key={idx} className="flex items-start gap-2">
                <span className="text-indigo-500">•</span>
                <span>{detail}</span>
              </li>
            ))}
          </ul>
        </div>

        {/* Examples (rich content) */}
        {section.examples && <div className="mb-4">{section.examples}</div>}

        {/* Questions */}
        <div className="space-y-3 mt-2">
          <h4 className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest flex items-center gap-2"> Test Your Knowledge
          </h4>
          {section.questions.map((q, idx) => (
            <InteractiveQuestion
              key={q.id}
              question={q.question}
              answer={q.answer}
              index={idx + 1}
            />
          ))}
        </div>
      </div>
    );
  },
  (prev, next) => prev.isHighlighted === next.isHighlighted
);

// ──────────────────────────────────────────────────────────────────────────────
// MAIN COMPONENT
// ──────────────────────────────────────────────────────────────────────────────
export const WritingMastery: React.FC = () => {
  const [isDarkMode, setIsDarkMode] = useState(false);
  const [inputValue, setInputValue] = useState('');
  const [highlightedId, setHighlightedId] = useState<string | null>(null);
  const [activeSectionIndex, setActiveSectionIndex] = useLessonState('section', 0);
  const [randomTip, setRandomTip] = useState<{ title: string; text: string } | null>(
    null
  );

  const searchInputRef = useRef<HTMLInputElement>(null);
  const listContainerRef = useRef<HTMLDivElement>(null);

  // Dark Mode detection
  useEffect(() => {
    const checkDarkMode = () => setIsDarkMode(document.documentElement.classList.contains('dark'));
    checkDarkMode();
    const observer = new MutationObserver(checkDarkMode);
    observer.observe(document.documentElement, { attributes: true, attributeFilter: ['class'] });
    return () => observer.disconnect();
  }, []);

  // Filter sections based on search query
  const filteredSections = useMemo(() => {
    if (!inputValue.trim()) return SECTIONS_DATA;
    const query = inputValue.toLowerCase();
    return SECTIONS_DATA.filter(
      (s) =>
        s.title.toLowerCase().includes(query) ||
        s.description.toLowerCase().includes(query) ||
        s.details.some((d) => d.toLowerCase().includes(query)) ||
        s.questions.some((q) => q.question.toLowerCase().includes(query))
    );
  }, [inputValue]);

  // Determine which section to show
  const visibleSections = useMemo(() => {
    if (filteredSections.length === 0) return [];
    if (inputValue.trim()) return filteredSections;
    const active = SECTIONS_DATA[activeSectionIndex];
    return active ? [active] : [];
  }, [filteredSections, activeSectionIndex, inputValue]);

  // Debounced search highlight
  useEffect(() => {
    if (!inputValue.trim()) {
      setHighlightedId(null);
      return;
    }
    const timer = setTimeout(() => {
      const query = inputValue.toLowerCase();
      const match = SECTIONS_DATA.find(
        (s) =>
          s.title.toLowerCase().includes(query) ||
          s.description.toLowerCase().includes(query) ||
          s.details.some((d) => d.toLowerCase().includes(query)) ||
          s.questions.some((q) => q.question.toLowerCase().includes(query))
      );
      if (match) {
        setHighlightedId(match.id);
        window.setTimeout(() => {
          const el = document.getElementById(`section-${match.id}`);
          if (el) {
            const y = el.getBoundingClientRect().top + window.scrollY - 120;
            window.scrollTo({ top: y, behavior: 'smooth' });
          }
        }, 0);
      } else {
        setHighlightedId(null);
      }
    }, 250);
    return () => clearTimeout(timer);
  }, [inputValue]);

  // Scroll to section when tab changes
  const scrollToSection = (index: number) => {
    setActiveSectionIndex(index);
    setHighlightedId(null);
    listContainerRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  };

  // Random tip on mount
  useEffect(() => {
    const tips = [
      {
        title: 'Did you know?',
        text: 'The average English speaker knows about 20,000 words, but a well‑read person may know up to 40,000! Reading widely is the best way to grow your vocabulary.',
      },
      {
        title: 'Pro Tip',
        text: 'When you learn a new word, write it in a sentence. This helps you remember its meaning and usage much better than just memorising a definition.',
      },
      {
        title: 'Memory Trick',
        text: 'To remember the difference between "there/their/they\'re": "There" has "here" (place); "Their" has "heir" (belonging); "They\'re" = "they are".',
      },
      {
        title: 'Common Mistake',
        text: 'Many students confuse "affect" and "effect". "Affect" is usually a verb (to influence), and "effect" is usually a noun (a result).',
      },
    ];
    setRandomTip(tips[Math.floor(Math.random() * tips.length)]);
  }, []);

  const refreshRandomTip = () => {
    const tips = [
      {
        title: 'Did you know?',
        text: 'The average English speaker knows about 20,000 words, but a well‑read person may know up to 40,000! Reading widely is the best way to grow your vocabulary.',
      },
      {
        title: 'Pro Tip',
        text: 'When you learn a new word, write it in a sentence. This helps you remember its meaning and usage much better than just memorising a definition.',
      },
      {
        title: 'Memory Trick',
        text: 'To remember the difference between "there/their/they\'re": "There" has "here" (place); "Their" has "heir" (belonging); "They\'re" = "they are".',
      },
      {
        title: 'Common Mistake',
        text: 'Many students confuse "affect" and "effect". "Affect" is usually a verb (to influence), and "effect" is usually a noun (a result).',
      },
    ];
    setRandomTip(tips[Math.floor(Math.random() * tips.length)]);
  };

  // ─── Sticky Navigation ────────────────────────────────────────────────────
  const NavTabs = () => (
    <div className="sticky top-0 z-30 w-full border-b-2 border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-[#0a0a0b]/95 py-2.5 backdrop-blur-md shadow-xs">
      <div className="mx-auto px-[5px] sm:px-6 md:px-8">
        <div className="flex w-full items-center gap-2 overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {SECTION_TABS.map((tab, idx) => {
            const isActive = activeSectionIndex === idx && !inputValue.trim();
            return (
              <button
                key={tab.id}
                onClick={() => scrollToSection(idx)}
                className={`shrink-0 whitespace-nowrap rounded-2xl px-4 py-2 text-xs font-black transition-all active:translate-y-0.5 ${
                  isActive
                    ? 'bg-orange-500 border-b-4 border-orange-700 text-white shadow-sm'
                    : 'border-2 border-b-4 border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-[#1a1a1a] text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:border-slate-300'
                }`}
              >
                {tab.label}
              </button>
            );
          })}
        </div>
      </div>
    </div>
  );

  // ─── Main container classes ─────────────────────────────────────────────
  const containerClasses = isDarkMode
    ? 'min-h-screen bg-[#0a0a0b] text-slate-200'
    : 'min-h-screen bg-slate-50 text-slate-900';

  return (
    <div className={containerClasses}>
      {/* ─── Duolingo Gradient Header ───────────────────────────────────────── */}
      <header className="relative overflow-hidden bg-gradient-to-r from-orange-500 via-amber-600 to-yellow-600 border-b-4 border-orange-800 pb-8 pt-10 text-white shadow-md">
        <div className="pointer-events-none absolute -right-12 -top-12 h-64 w-64 rounded-full bg-white/10 blur-2xl" />
        <div className="pointer-events-none absolute -left-12 -bottom-12 h-64 w-64 rounded-full bg-black/10 blur-2xl" />

        <div className="mx-auto px-[5px] sm:px-6 md:px-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2.5">
              <span className="inline-flex items-center justify-center rounded-2xl px-3.5 py-1 text-xs font-black tracking-wider uppercase bg-orange-400/30 text-white border border-orange-200/40 shadow-xs">
                VOCABULARY DEVELOPMENT
              </span>
              <span className="rounded-2xl bg-white/20 px-3 py-1 text-xs font-bold text-white/90 backdrop-blur-xs">
                ZJC Form 1 • English
              </span>
            </div>

            <div className="flex items-center gap-2 text-xs font-black text-white/90">
              <span className="inline-flex items-center gap-1.5 rounded-2xl bg-black/20 px-3.5 py-1.5 backdrop-blur-md border border-white/25 shadow-inner">
                📚 {SECTIONS_DATA.length} sections
              </span>
              <span className="inline-flex items-center gap-1.5 rounded-2xl bg-black/20 px-3.5 py-1.5 backdrop-blur-md border border-white/25 shadow-inner">
                ✨ {SECTIONS_DATA.reduce((acc, s) => acc + s.questions.length, 0)} questions
              </span>
            </div>
          </div>

          <h1 className="mt-4 mb-2 text-3xl sm:text-4xl md:text-5xl font-black tracking-tight text-white drop-shadow-sm">
            Vocabulary Development{' '}
            <span className="text-amber-200 font-bold italic">
              Growing Your Language
            </span>
          </h1>
          <p className="max-w-3xl text-sm sm:text-base leading-relaxed text-white/90 font-medium">
            Learn how words are built, understand word relationships, master formal
            and informal language, and discover idioms and proverbs. Expand your
            vocabulary and become a more confident communicator.
          </p>

          {/* Duolingo Search Bar */}
          <div className="mt-6 max-w-xl">
            <div className="flex items-center bg-black/20 backdrop-blur-md rounded-2xl border-2 border-white/25 focus-within:border-white focus-within:bg-black/30 transition-all shadow-inner">
              <Search className="ml-4 text-white/70" size={18} />
              <input
                ref={searchInputRef}
                type="text"
                value={inputValue}
                onChange={(e) => setInputValue(e.target.value)}
                placeholder="Search for a word‑building technique, idiom, or question..."
                className="w-full bg-transparent border-none outline-none py-2.5 px-3 text-sm text-white placeholder-white/60 font-medium"
              />
              {inputValue && (
                <button
                  onClick={() => {
                    setInputValue('');
                    setHighlightedId(null);
                    searchInputRef.current?.focus();
                  }}
                  className="mr-3 p-1.5 hover:bg-white/20 rounded-full transition-colors"
                >
                  <X size={16} className="text-white/80" />
                </button>
              )}
            </div>
          </div>
        </div>
      </header>

      {/* ─── Duolingo Sticky Navigation ──────────────────────────────────── */}
      <NavTabs />

      {/* ─── Main Content ────────────────────────────────────────────────── */}
      <div className="mx-auto px-[5px] sm:px-6 md:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8">
          {/* List of sections */}
          <div ref={listContainerRef} className="space-y-6">
            <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-4 text-sm text-slate-600 shadow-sm dark:border-slate-800 dark:bg-[#121212] dark:text-slate-300">
              <span className="font-semibold text-slate-900 dark:text-slate-100">
                {inputValue.trim()
                  ? `Search results (${filteredSections.length})`
                  : `${SECTION_TABS[activeSectionIndex]?.label || ''}`}
              </span>
              <span>{visibleSections.length} shown</span>
            </div>

            {visibleSections.length > 0 ? (
              visibleSections.map((section) => (
                <SectionCard
                  key={section.id}
                  section={section}
                  isHighlighted={section.id === highlightedId}
                />
              ))
            ) : (
              <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center text-slate-500 dark:border-slate-700 dark:bg-[#121212] dark:text-slate-400">
                No sections match your search.
              </div>
            )}

            {/* Duolingo-Styled Next / Previous Navigation Footer */}
            {!inputValue.trim() && (
              <div className="mt-8 flex items-center justify-between border-t-2 border-slate-200 dark:border-slate-800 pt-6">
                <button
                  onClick={() => scrollToSection(Math.max(0, activeSectionIndex - 1))}
                  disabled={activeSectionIndex === 0}
                  className="rounded-2xl border-2 border-b-4 border-slate-300 dark:border-slate-700 bg-white dark:bg-[#18181b] px-5 py-2.5 text-xs sm:text-sm font-black text-slate-700 dark:text-slate-300 shadow-sm transition hover:bg-slate-50 dark:hover:bg-[#27272a] active:translate-y-0.5 disabled:opacity-40 disabled:active:translate-y-0"
                >
                  ← Previous
                </button>
                <span className="text-xs font-black tracking-wider text-slate-400">
                  {activeSectionIndex + 1} / {SECTION_TABS.length}
                </span>
                <button
                  onClick={() => scrollToSection(Math.min(SECTION_TABS.length - 1, activeSectionIndex + 1))}
                  disabled={activeSectionIndex === SECTION_TABS.length - 1}
                  className="rounded-2xl border-2 border-b-4 border-orange-700 bg-orange-500 px-6 py-2.5 text-xs sm:text-sm font-black text-white shadow-sm transition hover:bg-orange-600 active:translate-y-0.5 disabled:opacity-40 disabled:active:translate-y-0"
                >
                  Next →
                </button>
              </div>
            )}
          </div>

          {/* ─── Sidebar ──────────────────────────────────────────────────── */}
          <aside className="space-y-6 lg:sticky lg:top-24 h-fit">
            {/* Random Tip Card */}
            <div className="rounded-2xl border border-indigo-100 dark:border-indigo-900/30 bg-white dark:bg-[#121212] p-5 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-bold text-indigo-600 dark:text-indigo-400">
                  💡 Vocabulary Tip
                </h3>
                <button
                  onClick={refreshRandomTip}
                  className="p-1.5 rounded-full hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors"
                >
                  <RefreshCw size={16} className="text-indigo-500 dark:text-indigo-400" />
                </button>
              </div>
              {randomTip && (
                <div className="space-y-2">
                  <p className="text-sm font-bold text-slate-800 dark:text-slate-100">
                    {randomTip.title}
                  </p>
                  <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                    {randomTip.text}
                  </p>
                </div>
              )}
            </div>

            {/* Quick Stats */}
            <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#121212] p-5 shadow-sm">
              <h3 className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-3">
                📊 Quick Stats
              </h3>
              <ul className="space-y-2 text-sm text-slate-600 dark:text-slate-400">
                <li className="flex justify-between">
                  <span>Vocabulary Topics</span>
                  <span className="font-bold text-indigo-600 dark:text-indigo-400">6</span>
                </li>
                <li className="flex justify-between">
                  <span>Total Questions</span>
                  <span className="font-bold text-indigo-600 dark:text-indigo-400">
                    {SECTIONS_DATA.reduce((acc, s) => acc + s.questions.length, 0)}
                  </span>
                </li>
                <li className="flex justify-between">
                  <span>Idioms &amp; Proverbs</span>
                  <span className="font-bold text-green-600 dark:text-green-400">9</span>
                </li>
              </ul>
            </div>

            {/* Quick Reminder */}
            <div className="rounded-2xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 p-5 shadow-sm">
              <h4 className="font-bold text-amber-800 dark:text-amber-300 mb-2">
                📝 Remember
              </h4>
              <p className="text-sm text-amber-700 dark:text-amber-300 leading-relaxed">
                A rich vocabulary helps you express yourself more precisely.
                Learn words in context, practice using them, and don't be afraid
                to ask about unfamiliar words.
              </p>
            </div>
          </aside>
        </div>
      </div>

      {/* ─── Floating Scroll-to-Top ──────────────────────────────────────── */}
      <div className="fixed bottom-6 right-6 z-50">
        <button
          onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
          className="w-12 h-12 bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600 text-white rounded-xl shadow-lg hover:shadow-indigo-500/25 transition-all duration-300 hover:scale-105 active:scale-95 flex items-center justify-center"
        >
          <ChevronUp size={22} />
        </button>
      </div>

      {/* ─── Key Takeaways Footer ────────────────────────────────────────── */}
      <div className="mx-auto px-[5px] sm:px-6 md:px-8 pb-12">
        <div className="mt-8 p-6 bg-gradient-to-r from-indigo-600 to-indigo-800 rounded-2xl text-white shadow-lg">
          <h3 className="font-bold text-xl mb-3">Key Takeaways</h3>
          <ul className="space-y-2 text-indigo-100 text-sm">
            <li className="flex items-start gap-2">
              <span className="text-indigo-300 font-bold">•</span>
              <span>
                <strong className="text-white">Word Building:</strong> Root words,
                prefixes, suffixes, and compound words help you decode new vocabulary.
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-indigo-300 font-bold">•</span>
              <span>
                <strong className="text-white">Word Relationships:</strong> Synonyms
                (same), Antonyms (opposite), Homophones (same sound), Homonyms
                (same spelling) – know the difference.
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-indigo-300 font-bold">•</span>
              <span>
                <strong className="text-white">Formal vs Informal:</strong> Use the
                right tone for the right audience – formal for school/work, informal
                for friends.
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-indigo-300 font-bold">•</span>
              <span>
                <strong className="text-white">Idioms &amp; Proverbs:</strong> Add
                colour and wisdom to your language. Learn common ones and their
                meanings.
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-indigo-300 font-bold">•</span>
              <span>
                <strong className="text-white">Context is Key:</strong> Always learn
                new words in sentences to understand their usage and meaning.
              </span>
            </li>
          </ul>
        </div>
      </div>
    </div>
  );
};

export default WritingMastery;
