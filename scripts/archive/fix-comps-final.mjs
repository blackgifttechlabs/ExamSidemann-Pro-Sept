...
interface CableToolRowProps { tool: string; description: string; }
const CableToolRow: React.FC<CableToolRowProps> = ({ tool, description }) => (
...
interface ComponentCardProps { title: string; description: string; icon: React.ReactNode; }
...
interface ToolCardProps { name: string; description: string; icon: React.ReactNode; }
const ToolCard: React.FC<ToolCardProps> = ({ name, description, icon }) => (
  <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 text-center hover:shadow-md transition-shadow">
    <div className="flex justify-center mb-3 text-purple-500">{icon}</div><h4 className="font-bold text-gray-900 dark:text-white mb-2">{name}</h4><p className="text-sm text-gray-600 dark:text-gray-400">{description}</p>
  </div>
);
...
`;
  ...
  if (file.includes('LearningOutcome4')) {
    comps = `interface StepCardProps { number: number; title: string; desc: string; }
const StepCard: React.FC<StepCardProps> = ({ number, title, desc }) => (
  <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border-l-4 border-emerald-500 shadow-sm">
    <div className="flex gap-4"><div className="bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 w-8 h-8 rounded-full flex items-center justify-center font-bold shrink-0">{number}</div><div><h4 className="font-bold text-gray-900 dark:text-white">{title}</h4><p className="text-sm text-gray-600 dark:text-gray-400 mt-1">{desc}</p></div></div>
  </div>
);

interface TroubleshootingMethodProps { name: string; desc: string; icon: React.ReactNode; }
const TroubleshootingMethod: React.FC<TroubleshootingMethodProps> = ({ name, desc, icon }) => (
  <div className="bg-emerald-50 dark:bg-emerald-900/10 p-5 rounded-xl border border-emerald-200 dark:border-emerald-800/50 text-center">
    <div className="inline-flex justify-center items-center w-12 h-12 bg-white dark:bg-gray-800 rounded-full text-emerald-500 mb-3 shadow-sm">{icon}</div><h4 className="font-bold text-gray-900 dark:text-white mb-2">{name}</h4><p className="text-sm text-gray-600 dark:text-gray-400">{desc}</p>
  </div>
);
