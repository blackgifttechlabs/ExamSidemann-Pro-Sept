...
const dir = path.join(__dirname, 'src/features/courses/polytechnic/nc-it/programming-concepts');
...
for (const file of files) {
  ...
  // Add Lucide icons
  const requiredIcons = ['CheckCircle', 'Rocket', 'Brain', 'FileCode', 'Code'];
...
\`;
      ...
      // In case it's not strictly 'export const LearningOutcome2: React.FC = () => {'
     const componentRegexAlt = new RegExp(\`export const LearningOutcome\${num} = \\(\\) => \\{\\n\`);
     if (content.match(componentRegexAlt)) {
         if (!content.includes('const [isDarkMode, setIsDarkMode] = useState')) {
        const hooksStr = \`export const LearningOutcome\${num} = () => {
  const [isDarkMode, setIsDarkMode] = useState(false);
...
\`;
        content = content.replace(componentRegexAlt, hooksStr);
    }
     }
  }

  // Rewrite entire container classes and structure if needed to make it edge-to-edge?
  // database administration used containerClasses = "w-full max-w-7xl mx-auto ..."
  // the old one here uses <div className="w-full space-y-12 text-left animate-dropdown-reveal px-[20px] pb-40">
  // That's actually fine, it's just the header that's polished.
