...
interface CableToolRowProps { tool: string; description?: string; cableType?: string; whatItFinds?: string; }
const CableToolRow: React.FC<CableToolRowProps> = ({ tool, description, cableType, whatItFinds }) => (
  <tr className="border-b border-gray-200 dark:border-gray-700"><td className="p-3 font-semibold text-gray-900 dark:text-white bg-gray-50 dark:bg-gray-800/50">{tool}</td><td className="p-3 text-sm">{description || cableType} {whatItFinds}</td></tr>
);

interface ElementRowProps { element: string; description?: string; whatToLookFor?: string; }
const ElementRow: React.FC<ElementRowProps> = ({ element, description, whatToLookFor }) => (
  <tr className="border-b border-gray-200 dark:border-gray-700"><td className="p-3 font-semibold text-gray-900 dark:text-white bg-gray-50 dark:bg-gray-800/50">{element}</td><td className="p-3 text-sm">{description || whatToLookFor}</td></tr>
);

interface EthernetSpeedRowProps { standard: string; speed: string; cableInfo?: string; notes?: string; }
const EthernetSpeedRow: React.FC<EthernetSpeedRowProps> = ({ standard, speed, cableInfo, notes }) => (
  <tr className="border-b border-gray-200 dark:border-gray-700"><td className="p-3 font-mono text-sm font-bold">{standard}</td><td className="p-3 font-semibold text-purple-600 dark:text-purple-400">{speed}</td><td className="p-3 text-sm">{cableInfo || notes}</td></tr>
);

interface ComponentCardProps { title: string; description?: string; icon?: React.ReactNode; use?: string; children?: React.ReactNode; number?: number; analogy?: string; }
const ComponentCard: React.FC<ComponentCardProps> = ({ title, description, icon, number, analogy, children }) => (
  <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 flex gap-4 items-start">
    <div className="bg-purple-100 dark:bg-purple-900/30 p-2 rounded-lg text-purple-600 dark:text-purple-400 mt-1 shrink-0">{icon || number}</div><div><h4 className="font-bold text-gray-900 dark:text-white mb-1">{title}</h4><p className="text-sm text-gray-600 dark:text-gray-400">{description || analogy}</p><p className="text-sm text-gray-600 dark:text-gray-400">{children}</p></div>
  </div>
);

interface ComparisonRowProps { feature: string; rip?: string; ospf?: string; dvrp?: string; lsrp?: string; }
const ComparisonRow: React.FC<ComparisonRowProps> = ({ feature, rip, ospf, dvrp, lsrp }) => (
  <tr className="border-b border-gray-200 dark:border-gray-700"><td className="p-3 font-semibold bg-gray-50 dark:bg-gray-800/50">{feature}</td><td className="p-3">{rip || dvrp}</td><td className="p-3">{ospf || lsrp}</td></tr>
);

interface ToolCardProps { title?: string; name?: string; description: string; icon?: React.ReactNode; use?: string; pros?: string; }
const ToolCard: React.FC<ToolCardProps> = ({ title, name, description, icon, use, pros }) => (
  <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 text-center hover:shadow-md transition-shadow">
    <div className="flex justify-center mb-3 text-purple-500">{icon}</div><h4 className="font-bold text-gray-900 dark:text-white mb-2">{title || name}</h4><p className="text-sm text-gray-600 dark:text-gray-400">{description}</p><p className="text-sm text-gray-600 dark:text-gray-400">{use || pros}</p>
  </div>
);
...
[2,4].forEach(n => fixComp(loc + '/LearningOutcome' + n + '.tsx'));
