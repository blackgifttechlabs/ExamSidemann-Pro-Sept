...
for (let file of files) {
  ...
  const startRegex = /<div className="relative bg-\[#1e1e2e\] rounded-2xl shadow-2xl border border-white\/10 overflow-hidden transform group-hover:scale-\[1\.02\] transition-transform duration-500">/;
  const endMarker = '</div>\n              </div>\n            </div>';
  
  const regex = /<div className="relative group w-full max-w-lg">[\s\S]*?<div className="absolute -inset-1[\s\S]*?<\/div>\s*<div className="relative[\s\S]*?<div className="bg-\[#181825\][\s\S]*?<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/div>/;

  // Since regex might be tricky, we will simply split string manually.
  
  let p1 = content.indexOf('<div className="relative group w-full max-w-lg">');
  if (p1 === -1) continue;
  
  let substr = content.substring(p1);
  let p2 = substr.indexOf('</div>\n              </div>\n            </div>');
  if (p2 === -1) {
    p2 = substr.indexOf('</div>\r\n              </div>\r\n            </div>'); 
  }
  ...
  if (p2 === -1) {
     const p3 = substr.indexOf('</div>\\n          </div>');
     if (p3 !== -1) {
         p2 = p3 - 20; 
     }
  }

  // Find exact start and end mathematically
  let divCount = 0;
  let endIdx = -1;
  let inDiv = true;
  for(let i = p1; i < content.length; i++) {
     if (content.substr(i, 4) === '<div') divCount++;
     else if (content.substr(i, 5) === '</div') divCount--;
     
     if (content.substr(i, 5) === '</div' && divCount === 0) {
         endIdx = i + 6;
         break;
     }
  }
  ...
  if (key && contents[key]) {
      let replacement = '<div className="relative group w-full max-w-lg">\n' + 
      ...
      // It's safer to just replace everything between p1 and endIdx
      
      content = content.substring(0, p1) + replacement + content.substring(endIdx);
