...
for (const file of files) {
  ...
  content = content.replace(
    ...
    `const containerClasses = 'w-full py-4 sm:py-6 px-3 sm:px-4 md:px-8 bg-[#fdfdfd] dark:bg-[#050505] min-h-screen text-gray-900 dark:text-gray-100 font-sans leading-relaxed';`
  );

  // Replace sectionHeaderClasses
  content = content.replace(
    /const sectionHeaderClasses = [^;]+;/,
    `const sectionHeaderClasses = 'text-xl sm:text-2xl md:text-3xl font-extrabold text-[#000] dark:text-white mt-12 mb-6 tracking-tight uppercase border-b-2 border-gray-200 dark:border-white/10 pb-2 w-full';`
  );

  // Replace cardClasses
  content = content.replace(
    /const cardClasses = [^}]+};\n/,
    `const cardClasses = (color: string) => 'py-4 md:py-6 mb-8 border-l-4 ' + (color === 'orange' ? 'border-orange-500' : color === 'purple' ? 'border-purple-500' : color === 'green' ? 'border-green-500' : 'border-blue-500') + ' pl-4 sm:pl-6';\n`
  );
  ...
  content = content.replace(/ rounded-[a-[z2]+/g, '');
  ...
  // Fix the big dark sections, e.g. `<div className="p-8 bg-[#111] text-white shadow-2xl relative overflow-hidden">`
  content = content.replace(/bg-\[#111\] text-white/g, 'border-l-4 border-gray-800 dark:border-gray-300 pl-4 sm:pl-6 py-4');
